function [new_chol, new_stepsize_iter] = adapt_cov(curr_chol, curr_stepsize_iter, adapt_iter, iter, post_draws, adapt_diag, adapt_param)
    ...
    if ~isempty(the_indx) && the_indx>1 % If in list, but not first...
        the_start = 1 + (the_indx>1)*adapt_iter(the_indx-1); % Start of current adaptation window
        the_cov = cov(post_draws(the_start:iter,:)); % Var-cov matrix over current adaptation window
        the_cov = the_cov/(mean(sqrt(diag(the_cov)))^2); % Normalize average std deviation
        the_n = iter-the_start+1;
