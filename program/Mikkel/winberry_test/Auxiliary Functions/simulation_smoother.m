function [ll, smooth_means, smooth_draws, oo_new] = simulation_smoother(dat_file, smooth_vars, num_smooth_draws, num_burnin_periods, M_, oo_, options_)
    ...
    disp('Simulation smoother...');
    timer = tic;
    
    for iter_i=1:num_smooth_draws % For each draw...
        
        % Simulate fake data
        set_dynare_seed(randi(2^32));
        ...
        for iter_j=1:size(smooth_vars,1)
            ...
            smooth_draws{iter_i}.(the_var) = smooth_means.(the_var) + sim_results.(the_var) - sim_smooth_means.(the_var);
        end

    end
    
    fprintf('Simulation smoother time: %6.1f sec\n\n', toc(timer));
