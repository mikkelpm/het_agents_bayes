...
for i_param = 1:n_param
    ...
    if i_param == 1
        if is_data_gen == 1
            ...
            sim_struct_moments2 = load('simul_moments.mat');
            for i_Epsilon = 1:nEpsilon
                for i_Measure = 3
                    sim_struct_moments2.(sprintf('%s%d%d', 'smpl_m', i_Epsilon, i_Measure)) = nan(T,1);
                    % Set sample moments to missing for the thrid
                    % moments
                end
            end
            save('simul_moments2.mat', '-struct', 'sim_struct_moments2');
...
%                     M_.H = MH_old;
    ...
    loglikes_plot = [loglikes(:,[1 2 4]) loglikes_macro(:,1)];
    plot(param_vals,loglikes_plot-max(loglikes_plot,[],1),'o-');
    the_ylim = ylim;
    line(param_true*ones(1,2), the_ylim, 'Color', 'k', 'LineStyle', ':');
    ylim(the_ylim);
    legend({'FI micro', '3rd moments micro', '2nd moments micro', 'macro'}, 'Location', 'SouthWest');
    title(tex_param{i_param})
    savefig([l_param{i_param} '2'])
    ...
    save([l_param{i_param} '2.mat'],'loglikes','loglikes_micro','loglikes_macro','param_vals')
